  ...
  <div class="home">
    <el-container>
      <el-aside style="padding-right: 10px" width="60%">
        <div class="image">
          <el-image style="padding-left: 25% ;padding-top: 5%" :src="require('@/assets/hdoj.png')" :fit="fill">
          </el-image>
        </div>
        <div class="questions">
            ...
            <el-col :span="4">
              <div class="grid-content bg-purple">
                <el-select v-model="qTag" placeholder="标签" @change="tChange()">
                  <el-option v-for="item in qTagOptions" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
              </div>
            </el-col>
          ...
          <el-tag v-for="tag in tags" :key="tag.text" closable :type="tag.type" @close="handleClose(tag)">
            {{tag.text}}
          </el-tag>
        </div>
        <div class="question">
          <el-table @row-click="openDetails"
            ...
            style="width: 100%;padding-left: 25%" :row-class-name="tableRowClassName" max-height="530px">
            <el-table-column label="id" align="center" prop="id" v-if="false" />
            <el-table-column label="labels" align="center" prop="labels" v-if="false" />
            <el-table-column prop="state" label="状态" width="100%">
            </el-table-column>
            <el-table-column prop="name" label="题目" sortable width="300%">
            </el-table-column>
            <el-table-column prop="solution" label="题解" sortable width="100%">
            </el-table-column>
            <el-table-column prop="passingRate" label="通过率" sortable width="100%">
            </el-table-column>
            <el-table-column prop="difficulty" label="难度" sortable :sort-method="sortByDifficulty" width="150%">
          ...
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :default-page-size="3"
            :current-page="currentPage" :page-size="pageSize" layout="total, prev, pager, next, jumper"
            :total="tables.length" style="padding-left: 25%">
          </el-pagination>
      ...
      <el-main style="padding-right: 15%">
        <el-calendar style="font-size: small" ref="calendar">
          <template #header="{ date }">
            <span>{{ date }}</span>
            <el-button-group>
              <el-button size="small" @click="select('prev-month')">上一月</el-button>
              <el-button size="small" @click="select('today')">本月</el-button>
              <el-button size="small" @click="select('next-month')">下一月</el-button>
            </el-button-group>
          </template>
        ...
        <div class="image">
          <el-image :src="require('@/assets/toMaterial.png')" :fit="fill" @click="toMaterial(0)"></el-image>
        </div>
